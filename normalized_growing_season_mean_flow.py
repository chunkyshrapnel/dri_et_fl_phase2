...
for site in site_list:
    ...
    # Normalize the Data
    mean = df_station['discharge_mean_cfs'].mean()
    #df_station['discharge_mean_cfs'].apply(lambda x: x / mean, inplace=True)
    df_station = df_station.assign(discharge_mean_cfs=lambda x: (x['discharge_mean_cfs'] / mean))
...
circle = p.circle(x='discharge_mean_cfs', y='gs_etof',
...
regression_line = np.polyfit(df['discharge_mean_cfs'], df['gs_etof'], 1, full=True)
slope = regression_line[0][0]
intercept = regression_line[0][1]
y_predicted = [slope * i + intercept for i in df['discharge_mean_cfs']]
p.line(df['discharge_mean_cfs'], y_predicted, color='black')

# Calculations to be used in the stats label on every scatter plot
pearson_r, pearson_p = stats.pearsonr(df['gs_etof'], df['discharge_mean_cfs'])
kendall_r, kendall_p = stats.kendalltau(df['gs_etof'], df['discharge_mean_cfs'])
...
hover.tooltips = [
    ...
    ('Normalized Growing Season Mean Discharge', '@discharge_mean_cfs')
]
p.add_tools(hover)
